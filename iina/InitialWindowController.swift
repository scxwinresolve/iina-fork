...
class InitialWindowController: NSWindowController {
  ...
  override func windowDidLoad() {
    ...
    // 春星开讲定制版标识
    let forkLabel = NSTextField(labelWithString: "春星开讲定制版")
    forkLabel.font = NSFont.systemFont(ofSize: 11, weight: .light)
    forkLabel.textColor = .secondaryLabelColor
    forkLabel.translatesAutoresizingMaskIntoConstraints = false
    if let superview = versionLabel.superview {
      superview.addSubview(forkLabel)
      NSLayoutConstraint.activate([
        forkLabel.topAnchor.constraint(equalTo: versionLabel.bottomAnchor, constant: 6),
        forkLabel.centerXAnchor.constraint(equalTo: versionLabel.centerXAnchor)
      ])
    }
