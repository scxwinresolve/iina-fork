...
fileprivate class EqualizerView: NSView {
  ...
  init(player: PlayerCore) {
    ...
    // 伽马预设按钮
    let presetButtonA = NSButton(title: "709-A", target: self, action: #selector(presetAction(_:)))
    presetButtonA.tag = 0
    presetButtonA.controlSize = .small
    presetButtonA.bezelStyle = .rounded
    presetButtonA.setContentHuggingPriority(.init(150), for: .horizontal)
    let presetButton22 = NSButton(title: "伽马2.2", target: self, action: #selector(presetAction(_:)))
    presetButton22.tag = 1
    presetButton22.controlSize = .small
    presetButton22.bezelStyle = .rounded
    presetButton22.setContentHuggingPriority(.init(150), for: .horizontal)
    let presetButton1886 = NSButton(title: "BT.1886（2.4）", target: self, action: #selector(presetAction(_:)))
    presetButton1886.tag = 2
    presetButton1886.controlSize = .small
    presetButton1886.bezelStyle = .rounded
    presetButton1886.setContentHuggingPriority(.init(150), for: .horizontal)
    let presetRow = ui.hStack(spacing: 8, presetButtonA, presetButton22, presetButton1886)
  ...
  @objc private func presetAction(_ sender: NSButton) {
    ...
    case 2: value = Self.gammaPreset1886
    default: return
    }
